#!/usr/bin/env bash
# cleanup_temp_files.sh - Clean up temporary files on macOS
...
clean_directory "$HOME/Library/Caches" "User cache directory"
clean_directory "$HOME/.Trash" "User trash"

# Safari caches (if exists)
...
if [[ -d "$HOME/Library/Caches/Google/Chrome" ]]; then
    clean_directory "$HOME/Library/Caches/Google/Chrome/Default/Cache" "Chrome cache"
fi

# Firefox caches (if exists)
FIREFOX_PROFILE=$(find "$HOME/Library/Caches/Firefox/Profiles" -maxdepth 1 -type d -name "*.default*" 2>/dev/null | head -1)
